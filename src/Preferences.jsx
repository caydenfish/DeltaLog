...
export default function Preferences({ value, onChange, fields, onApplyRestToAll, filterQuery }) {
  ...
  const [showUnitsGroup, setShowUnitsGroup] = useState(false);
  ...
  // A row inside the collapsed "Training Preferences" group should still
  // be findable by search without requiring a tap to expand the group first.
  const trainingPrefsOpen = showTrainingPrefs || (!!filterQuery && ["trainingIdeology", "scoreDisplay", "weightEntryMode", "plateSizes", "scientificNames", "restSeconds"].some(matches));
  const unitsGroupOpen = showUnitsGroup || (!!filterQuery && ["units", "timeFormat"].some(matches));
  ...
  const { units, muscleNameMode, scoreDisplay, weightEntryMode, restSeconds, trainingIdeology } = state;
  // Grouping into a "Training Preferences" sub-level only applies to the
  // full/unrestricted Settings usage (no `fields` prop). The in-workout
  // menu passes an explicit fields subset and keeps its current flat
  // layout — those need to stay one tap away mid-workout, not two.
  const grouped = !fields;

  return (
    <div style={{ background: T.surface, border: `1px solid ${T.line}`, borderRadius: 12, padding: 14 }}>
      {!grouped && show("units") && matches("units") && (
      ...
      {!grouped && show("timeFormat") && matches("timeFormat") && (
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
        <div>
      ...
      {/* Units — collapsible group, grouped/Settings usage only. Combines
          weight units and clock format under one "Units" heading instead
          of two flat top-level rows. */}
      {grouped && (!filterQuery || ["units", "timeFormat"].some(matches)) && (
        <div style={{ marginBottom: 14 }}>
          <button
            onClick={() => setShowUnitsGroup(!showUnitsGroup)}
          ...
          >
            <div style={{ textAlign: "left" }}>
              <div style={{ color: T.text, fontSize: 14 }}>Units</div>
              <div style={{ color: T.dim, fontSize: 11 }}>Weight units and clock format</div>
            </div>
            <div style={{ color: T.dim, fontSize: 16 }}>{unitsGroupOpen ? "▲" : "▼"}</div>
          </button>
      ...
      {!grouped && show("scientificNames") && matches("scientificNames") && (
      <div style={{ marginBottom: 14 }}>
      ...
      {grouped && (!filterQuery || ["trainingIdeology", "scoreDisplay", "weightEntryMode", "plateSizes", "scientificNames", "restSeconds"].some(matches)) ? (
        <div style={{ marginBottom: 14 }}>
          <button
          ...
          >
            <div style={{ textAlign: "left" }}>
              <div style={{ color: T.text, fontSize: 14 }}>Training Preferences</div>
              <div style={{ color: T.dim, fontSize: 11 }}>Training focus, strength score, set entry, big plates, muscle names, rest timer</div>
            </div>
            <div style={{ color: T.dim, fontSize: 16 }}>{trainingPrefsOpen ? "▲" : "▼"}</div>
          </button>
      ...
      {!grouped && show("restSeconds") && matches("restSeconds") && (
      <div>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ color: T.text, fontSize: 14 }}>Default rest timer</div>
        ...
        {onApplyRestToAll && confirmApplyAll && (
          <div style={{ marginTop: 10, background: T.surface2, border: `1px solid ${T.accent}`, borderRadius: 10, padding: 10 }}>
            ...
            <div style={{ color: T.dim, fontSize: 11.5, marginBottom: 10, lineHeight: 1.4 }}>This overwrites any exercise-specific rest times you've set in this workout. Can't be undone.</div>
