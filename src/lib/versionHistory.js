...
export const VERSION_HISTORY = [
  {
    ...
    items: [
      ...
      "ExportWorkoutModal.jsx: fixed the reported flickering/glitching during \"Save image\" generation, confirmed to happen specifically during html2canvas's capture, not on open or while toggling options. Root cause: previewRef sits inside a scrollable ancestor (the modal's own overflowY:auto content div), and html2canvas clones the DOM off-screen to render it -- a clone doesn't carry over live scroll position, so a scrolled ancestor comes back scrollTop 0 in the clone, no longer matching where the live page measured the preview to be. That live/clone mismatch is what was visibly snapping into place as flicker. Fix: added a containerRef on the scrollable div, reset to scrollTop 0 immediately before calling html2canvas, and passed scrollX: 0, scrollY: 0 so html2canvas doesn't layer its own window-scroll offset on top of that -- live and cloned layouts now agree, nothing left to visibly snap.",
    ],
  },
  {
    version: "1.10.3",
